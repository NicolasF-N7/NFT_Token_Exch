...
const compiledToken = require('../compiled/TokenManagerMetadataEnum.json');
let contractABI = compiledToken['abi'];
let contractByteCode = '0x'+compiledToken['evm'].bytecode.object;

let _name = "Nexchange";
let _symbol = "NC";
let _uriBase = "http://localhost/nexchange/tokens/";
...
beforeEach(async () => {
    ...
    token = await new web3.eth.Contract(contractABI)
        .deploy({
            ...
            arguments: [_name, _symbol, _uriBase]
...
describe('Token Contract', () => {
    ...
    //=====Metadata=====
    //Verify the metadata name of tokens
    it('Metadata Name', async () => {
      let name_reported = await token.methods.name().call();
      assert(name_reported = _name);
    });

    //same for symbol of tokens
    it('Metadata Symbol', async () => {
      let symbol_reported = await token.methods.symbol().call();
      assert(symbol_reported = _symbol);
    });
    ...
    it('Metadata Symbol', async () => {
      ...
      let tokenMedatadaUri = await token.methods.tokenURI(tokenId1).call();
      assert(tokenMedatadaUri = _uriBase + tokenId1);
    });
